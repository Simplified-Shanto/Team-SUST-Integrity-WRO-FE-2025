...
while True:
    ...
    for cnt in contours_blue:
        ...
        if w*h > thresholdArea:  # Adjust 15 based on resolution (this filters out thin junk)
            blue_line_count += 1
            cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)

    for cnt in contours_orange:
        x, y, w, h = cv2.boundingRect(cnt)
        if w*h > thresholdArea:
            orange_line_count += 1
    ...
    cv2.putText(frame, f"Orange Lines: {total_linecounted}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 140, 255), 2)

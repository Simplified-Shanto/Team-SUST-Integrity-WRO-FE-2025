...
# Create a window to host the trackbars
cv2.namedWindow("Trackbars", cv2.WINDOW_NORMAL)
cv2.resizeWindow("Trackbars", 600, 300)

# --- IMPORTANT: Give the window time to render ---
...
while True:
    ...
    # Calculate actual FPS for debugging (optional)
    ...
    processing_time = (time.time() - new_frame_time) * 1000 # Convert to milliseconds

    # Calculate the remaining time to wait to hit target_fps
    wait_time = max(1, int(frame_delay - processing_time)) # Ensure at least 1ms wait
